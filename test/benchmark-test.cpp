...
void lookup_int_rawptr_hashmap(const RawPtr::Hashmap<int, int>& hashmap, int num_items) {
    for (int i = 0; i < num_items; ++i) {
...
void lookup_int_sharedptr_hashmap(const SharedPtr::Hashmap<int, int>& hashmap, int num_items) {
    for (int i = 0; i < num_items; ++i) {
        auto it = hashmap.at(i);
        benchmark::DoNotOptimize(it);
    }
...
void erase_int_unordered_map(std::unordered_map<int, int>& hashmap, int num_items) {
    for (int i = 0; i < num_items; ++i) {
        auto it = hashmap.find(i);
        if (it != hashmap.end()) {
            hashmap.erase(it);  // Erase the element found
        }
    }
}

void erase_int_rawptr_hashmap(RawPtr::Hashmap<int, int>& hashmap, int num_items) {
    for (int i = 0; i < num_items; ++i) {
        hashmap.erase(i);
    }
}

void erase_int_sharedptr_hashmap(SharedPtr::Hashmap<int, int>& hashmap, int num_items) {
    for (int i = 0; i < num_items; ++i) {
        hashmap.erase(i);
    }
}
...
static void unordered_map_erase(benchmark::State& state) {
    ...
    for (auto _ : state) {
        erase_int_unordered_map(hashmap, num_items);
    }
}

static void rawptr_hashmap_erase(benchmark::State& state) {
    RawPtr::Hashmap<int, int> hashmap;
    int num_items = state.range(0);
    for (int i = 0; i < num_items; ++i) {
        hashmap.insert(i, i);
    ...
    for (auto _ : state) {
        erase_int_rawptr_hashmap(hashmap, num_items);
    }
}

static void sharedptr_hashmap_erase(benchmark::State& state) {
    SharedPtr::Hashmap<int, int> hashmap;
    int num_items = state.range(0);
    for (int i = 0; i < num_items; ++i) {
        hashmap.insert(i, i);
    ...
    for (auto _ : state) {
        erase_int_sharedptr_hashmap(hashmap, num_items);
    }
}
...
BENCHMARK(unordered_map_erase) -> Arg(1) -> Arg(16) -> Arg(256) -> Arg(4096);

BENCHMARK(rawptr_hashmap_erase) -> Arg(1) -> Arg(16) -> Arg(256) -> Arg(4096);

BENCHMARK(sharedptr_hashmap_erase) -> Arg(1) -> Arg(16) -> Arg(256) -> Arg(4096);
/*
